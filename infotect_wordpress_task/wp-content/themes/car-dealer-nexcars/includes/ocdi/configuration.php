...
function ocdi_import_files() {
	...
	if(file_exists(get_stylesheet_directory() . '/includes/ocdi/preview/'.$pac_name)) {
		return [
			[
				'import_file_name'           => 'Car Dealer NexCars',
				'local_import_file'            =>  get_stylesheet_directory() . '/includes/ocdi/preview/'.$pac_name,
				'import_preview_image_url'   => get_stylesheet_directory_uri() . '/includes/ocdi/preview/preview_cars.jpg',
				'preview_url'                => 'https://demo.wpdirectorykit.com/car-dealer-nexcars',
			]
		];
	} elseif(file_exists(ABSPATH."ocdi/car-dealer-nexcars/".$pac_name)) {
		return [
			[
				'import_file_name'           => 'Car Dealer NexCars',
				'local_import_file'            => ABSPATH."ocdi/car-dealer-nexcars/".$pac_name,
				'import_preview_image_url'   => get_stylesheet_directory_uri() . '/includes/ocdi/preview/preview_cars.jpg',
				'preview_url'                => 'https://demo.wpdirectorykit.com/car-dealer-nexcars',
			]
		];
		...
		return [
			[
				'import_file_name'           => 'Car Dealer NexCars',
				'import_file_url'            => 'https://demo.wpdirectorykit.com/car-dealer-nexcars/'.$pac_name,
				'import_preview_image_url'   => get_stylesheet_directory_uri() . '/includes/ocdi/preview/preview_cars.jpg',
				'preview_url'                => 'https://demo.wpdirectorykit.com/car-dealer-nexcars',
			]
		];
...
function ocdi_after_import_setup($selected_import) {
	...
	if ( true) {
		if($results_page_id) {
			...
			$to = car_dealer_nexcars_url_suffix(get_permalink($results_page_id), 'is_featured=on#results');
			car_dealer_nexcars_replace_links($from, $to);
		}
		...
		if($results_page_id) {
			...
			$to = car_dealer_nexcars_url_suffix(get_permalink($results_page_id), 'field_5=Sale#results');
			car_dealer_nexcars_replace_links($from, $to);
		}
    }
	...
	set_theme_mod( 'footer_content', esc_html__('Aenean sollicitudin, lorem quis bibend auctor, nisi elit consequat ipsum, necittis sem nibh id elit. Duis sed odio enim.', 'car-dealer-nexcars') );
	set_theme_mod( 'footer_phone_number', '(917) 382-2057' );
	set_theme_mod( 'footer_email_address', 'agent@info.com' );
	set_theme_mod( 'footer_copyright_text', 'Copyright © '.date('Y').' NEXCARS' );
	...
	if(true){
		...
		car_dealer_nexcars_insert_widget('sidebar-1', 'search');
		car_dealer_nexcars_insert_widget('sidebar-1', 'recent-posts');
		car_dealer_nexcars_insert_widget('sidebar-1', 'categories');
		...
		car_dealer_nexcars_insert_widget('sidebar', 'search');
		car_dealer_nexcars_insert_widget('sidebar', 'recent-posts');
		car_dealer_nexcars_insert_widget('sidebar', 'categories');
		...
		car_dealer_nexcars_insert_widget('footer', 'text', array('title' => esc_html__('Popular Properties', 'car-dealer-nexcars'), 'text'=>'[wdk-latest-listings-list]'));
		car_dealer_nexcars_insert_widget('footer', 'recent-posts');
		car_dealer_nexcars_insert_widget('footer', 'text', array('title' => esc_html__('Newsletter', 'car-dealer-nexcars'), 'text'=>'[eli-newsletter]'));
	}
	...
	if(true){
		...
		set_theme_mod('sign_in_button_text', esc_html__('Login', 'car-dealer-nexcars'));

		set_theme_mod('property_button_text', esc_html__('Add Car', 'car-dealer-nexcars'));
	}

}

function car_dealer_nexcars_replace_links($from = '', $to = '') {
	global $wpdb;
...
if(!function_exists('car_dealer_nexcars_insert_widget'))
{
    function car_dealer_nexcars_insert_widget($sidebar_id, $widget_name, $widget_options_new = array())
    {
...
if ( ! function_exists('car_dealer_nexcars_url_suffix'))
{
	function car_dealer_nexcars_url_suffix($base_url, $extension_url="")
	{
